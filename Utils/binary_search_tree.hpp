...
class Node
{
	public:
		...
		T 		val;
	private:
		int 			color; //max_size() correction (not a RBTREE implementation)
	...
	Node(): left(NULL), right(NULL), parent(NULL), val(){}
	
	Node(const T &data) : left(NULL), right(NULL), val(data){}

	Node(const T &val, Node *left, Node *right, Node *parent)
		: left(left), right(right), parent(parent), val(val){}

	~Node(){};

	Node(const Node &other)
		: left(other.left), right(other.right), parent(other.parent), val(other.val){}
...
class Tree
{
	...
	public:
		...
		bool insert (iterator position, const Pair& value)
		{
			...
			pointer pt = position.base();
			newNode->parent = pt;
			
			if ( !pt || !pt->parent
			|| comp(pt->parent->val, value) != comp(pt->val, value))
			{
			...
			if (pt == root)
				root = newNode;
			else if ((pt->left != nil && pt->left->val == value )
			|| (pt->right != nil &&pt->right->val == value))
			{
				alloc.destroy(newNode);
				alloc.deallocate(newNode, 1);
			}
			else if (comp(pt->val, value))
				pt->right = newNode;
			else if (comp(value, pt->val))
				pt->left = newNode;
			else
			{
				alloc.destroy(newNode);
			...
			std::cout << "inserted by hint" << std::endl;
		...
		bool erase(pointer const &pt)
		{
			...
			switch (pr.first)
			{
				...
				case 3:
				{
					...
					/*std::cout << pr.second->val.second 
					<< " p:" << pr.second->parent->val.second 
					<< " pr:" << pr.second->parent->right->val.second
					<< " l:" << pr.second->left->val.second
					<< " r:" << pr.second->right->val.second*/
					//<< std::endl;
